...
export const hiringService = {
    /**
     * Get available drivers for hiring (unemployed with complete profiles)
     */
    ...
    getEmployments: async (params = {}) => {
        try {
            ...
            const url = `${API_BASE_URL}/employments/company${queryString ? `?${queryString}` : ''}`;
    ...
    /**
     * Get available employed drivers for trip assignment (not in active trips)
     * @param {Object} params - Query parameters (e.g., { startDateTime, endDateTime })
     */
    getAvailableDriversForTrip: async (params = {}) => {
        try {
            const queryString = new URLSearchParams(params).toString();
            const url = `${API_BASE_URL}/employments/available${queryString ? `?${queryString}` : ''}`;
            const response = await fetch(url, {
                method: 'GET',
                headers: getAuthHeaders()
            });
